...
export const reportService={exportCSV(rows,filename='relatorio.csv'){const keys=Object.keys(rows[0]||{}),labels={id:'Identificador',user_id:'Usuário',created_at:'Criado em',updated_at:'Atualizado em',order_number:'Número do pedido',sale_date:'Data da venda',status:'Situação',product:'Produto',quantity:'Quantidade',total:'Faturamento',net_profit:'Lucro líquido',sku:'Código do produto',name:'Nome',category:'Categoria',cost_price:'Preço de custo',sale_price:'Preço de venda',stock:'Estoque',min_stock:'Estoque mínimo',margin:'Margem',profit:'Lucro',label:'Período ou item',revenue:'Faturamento',units:'Unidades',orders:'Pedidos',detail:'Detalhes',cost:'Custo'};const csv=[keys.map(k=>labels[k]||k).join(';'),...rows.map(r=>keys.map(k=>`"${String(r[k]??'').replaceAll('"','""')}"`).join(';'))].join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv'}));a.download=filename;a.click();URL.revokeObjectURL(a.href)}};
export { client };
